...
export default async function handler(req, res) {
  ...
  try {
    // Insert the form submission into the Contacts table
    await query(`
      INSERT INTO Contacts (Name, Email, Subject, Content, CreatedAt)
      VALUES (?, ?, ?, ?, NOW())
    ...
    res.status(201).json({ message: 'Message received, we will contact you shortly.' });
  } catch (error) {
    console.error(error);
